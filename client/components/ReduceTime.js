...
export const ReduceTime = () => {
  ...
  useEffect(() => {
    const videos = videoList.current;
    const time = [...videos.children];

    const dataTime = time
      .map((data) => {
        return data.dataset.time;
      })
      .map((time) => {
        const numberTime = time.split(":").map(parseFloat);

        return numberTime[0] * 60 + numberTime[1];
      })
    ...
    setTime((prevState) => {
      const hours = Math.floor(dataTime / 3600);
      const minutes = Math.floor((dataTime % 3600) / 60);
      const seconds = (dataTime % 3600) % 60;

      return { ...prevState, hours: hours, minutes: minutes, seconds: seconds };
    });
  }, [videoList, setTime]);
  ...
  return (
    <div>
      <div>
        <h1>Total time of all videos</h1>
        <h1>
          {time.hours} : {time.minutes} : {time.seconds}
        </h1>
      </div>
      <ul className="videos" ref={videoList}>
